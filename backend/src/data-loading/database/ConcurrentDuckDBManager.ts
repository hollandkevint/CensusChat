...
export class ConcurrentDuckDBManager extends EventEmitter {
  ...
  // Store actual connections separately
  private connectionMap: Map<string, DuckDBConnection> = new Map();

  // Request queues
  private readerQueue: Array<{ resolve: (result: { connection: DuckDBConnection; id: string }) => void; reject: (error: Error) => void; priority: number }> = [];
  private writerQueue: Array<{ resolve: (result: { connection: DuckDBConnection; id: string }) => void; reject: (error: Error) => void; priority: number }> = [];
  ...
  async initialize(): Promise<void> {
    ...
    try {
      console.log('Setting up DuckDB connection pools...');

      // Create DuckDB instance using fromCache for singleton management
      this.instance = await DuckDBInstance.fromCache(this.databasePath, {
        memory_limit: '4GB',
        threads: '4',
      });
  ...
  async query<T = any>(
    ...
    try {
      ...
      const queryPromise = this.executeQuery(connection, sql);
  ...
  async transaction<T = any>(
    ...
    try {
      ...
      for (const query of queries) {
        const result = await this.executeQuery(connection, query.sql);
        results.push(result);
  ...
  async batchInsert(
    ...
    for (let i = 0; i < data.length; i += batchSize) {
      ...
      try {
        ...
        await this.query(sql, [], { connection: 'writer', priority: 'high' });
  ...
  async close(): Promise<void> {
    ...
    for (const [id, connection] of this.connectionMap.entries()) {
      try {
    ...
    if (this.instance) {
      try {
        ...
        console.error('Error closing DuckDB instance:', error);
      }
      this.instance = null;
    }
  ...
  private async acquireConnection(
    ...
    priority: 'low' | 'normal' | 'high'
  ): Promise<{ connection: DuckDBConnection; id: string }> {
  ...
  private tryGetConnection(type: 'reader' | 'writer' | 'any'): { connection: DuckDBConnection; id: string } | null {
    ...
    if ((type === 'reader' || type === 'any') && this.availableReaders.size > 0) {
      ...
      const connectionInfo = this.readerPool.get(id)!;
    ...
    if ((type === 'writer' || type === 'any') && this.availableWriters.size > 0) {
      ...
      const connectionInfo = this.writerPool.get(id)!;
      connectionInfo.inUse = true;
      connectionInfo.lastUsedAt = new Date();
  ...
  private releaseConnection(connectionId: string): void {
    ...
    if (this.readerPool.has(connectionId)) {
      const connectionInfo = this.readerPool.get(connectionId)!;
    ...
    if (this.writerPool.has(connectionId)) {
      const connectionInfo = this.writerPool.get(connectionId)!;
      connectionInfo.inUse = false;
      connectionInfo.lastUsedAt = new Date();
  ...
  private async executeQuery(connection: DuckDBConnection, sql: string): Promise<any[]> {

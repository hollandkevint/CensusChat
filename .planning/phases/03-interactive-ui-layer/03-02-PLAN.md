...
<task type="auto">
  <name>Task 3: Integrate App Bridge into ChatInterface</name>
  <files>
    frontend/src/components/ChatInterface.tsx
...
**Add backend route for UI resources:**
Create simple endpoint in mcp.routes.ts that returns available UI resources with their HTML content.
  </action>
  <verify>
```bash
...
```
  ...
  <done>
ChatInterface renders AppBridge for data tables when UI resources available, falls back to static table otherwise
  </done>
</task>
...
8. TypeScript compiles: `cd frontend && npm run typecheck && cd ../backend && npm run typecheck`
</verification>

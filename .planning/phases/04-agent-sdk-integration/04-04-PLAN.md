...
2. Verify MCP HTTP configuration exists in mcpConfig.ts (lines 20-28):
   - `getMcpConfig()` returns `{ censuschat: { type: "http", url: "${baseUrl}/mcp" } }`
   - Default URL is `http://localhost:3001/mcp` (or MCP_SERVER_URL env var)
   - This provides the HTTP transport details needed for Agent SDK connection

3. Create `backend/src/agent/agentSdkService.ts` with:
   ...
   export async function queryWithAgentSdk(
     ...
     // mcpConfig.censuschat.url is http://localhost:3001/mcp (verified in mcpConfig.ts line 26)
     ...
     for await (const message of query({
       ...
       options: {
         mcpServers: {
           censuschat: {
             type: "http",
             url: mcpConfig.censuschat.url,  // Use full path: mcpConfig.censuschat.url

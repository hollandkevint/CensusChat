...
must_haves:
  truths:
    - "User can ask 'now filter to income > $75K' after prior query and system applies filter to same region"
    - "Comparison query 'Tampa Bay vs Phoenix' executes both regions in parallel"
  artifacts:
...
<task type="auto">
  ...
  <action>
Modify `AgentService.query()` (starting at line 301) to retrieve and inject session context:

1. At the start of `query()` method (line 301-303), retrieve previous session using getSessionIdForUser() which exists at sessionManager.ts line 103-112:
   ```typescript
   async query(prompt: string): Promise<AgentQueryResult<QueryResponse | ComparisonResponse>> {
     ...
     // Use contextPrompt instead of prompt in subsequent calls below...
   }
   ```
...
3. Update BOTH query paths to use `contextPrompt` instead of `prompt`:
   - **Line 306**: In `queryComparison()` call, change `queryComparison(prompt, {...})` to `queryComparison(contextPrompt, {...})`
   - **Line 324**: In `queryCensus()` call, change `queryCensus(prompt, {...})` to `queryCensus(contextPrompt, {...})`

4. Import SessionContext type from sessionManager.ts (already exported at line 7).

Note: Keep the existing storeSession() calls (lines 312-317, 331-336) - they continue to store results for the next query.
  </action>
  <verify>
...
<task type="auto">
  ...
  <action>
1. In `agentSdkService.ts`, add parallel comparison function using Promise.all pattern:

   NOTE: The Agent SDK `query()` function does NOT support the Task tool for spawning subagents - that's a Claude Code feature, not available in the SDK. Use Promise.all pattern.
   ...
   export async function queryComparisonParallel(
     ...
     if (extractedRegions.length < 2) {
       ...
       return { success: result.success, comparison: { regions: [], summary: "", differences: [] }, explanation: result.result?.toString() || "" };
     }
     ...
     return {
       ...
       comparison: {
         regions: results.map((r, i) => ({
           ...
           total_population: extractPopulation(r),
           // ... other fields extracted from result
         })),
...
2. In `agentService.ts`, update `queryComparison()` to use parallel execution:
   - If USE_AGENT_SDK env var is true, call `queryComparisonParallel()` from agentSdkService
   - Otherwise, add Promise.all pattern directly for parallel execution
...
4. Helper function `combineIntoComparison()` in agentService.ts:
   ```typescript
  ...
  <verify>
    - `grep -n "Promise.all" backend/src/agent/agentSdkService.ts` shows parallel execution pattern
  ...
  <done>
    Comparison queries use parallel execution via Promise.all pattern. "Compare Tampa Bay vs Phoenix" runs both region queries simultaneously.
  </done>
</task>
